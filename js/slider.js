...
function initializeSlider(slider, display) {
    dist = parseInt(slider.getAttribute('distance')) // The allowed slider movement in pixels.
    slider.distance = dist ? dist : carpeDefaultSliderLength // Deafault distance from global var.
    from = parseFloat(display.getAttribute('from')) // Min/start value for the display.
    from = from ? from : 0 // Default min/start value: 0.
...
function updateSliderToMatchDisplay(slider, display) {
...
{
    if (mouseover) {
	updateSliderToMatchDisplay(slider, display)
	mouseover = false // Stop the sliding.
	if (sliderChangeCallback) {
	    sliderChangeCallback(v, slider);
	}
    }
...
function setSliderChangeCallback(callback) {
...
window.onload = function() // Set up the sliders and the displays when the document is ready
{
